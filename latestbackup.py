...
def rebuild_vectorstore():
    ...
    # Corrected: Create and persist vectorstore properly
...
documents_dir = "/Users/krithi/Desktop/EBT-v3/documents"
db_dir = "/Users/krithi/Desktop/EBT-v3/db"
...
def get_rag_chain(category: str):
    def _invoke(input):
        ...
        placeholders = extract_placeholders(input["email"])
    ...
    return RunnableLambda(_invoke) | model | StrOutputParser()
...
full_chain = (
    ...
    | RunnableBranch(
        (lambda x: x["category"] == "leave_request", get_rag_chain("leave_request")),
        (lambda x: x["category"] == "job_inquiry", get_rag_chain("job_inquiry")),
        (lambda x: x["category"] == "onboarding", get_rag_chain("onboarding")),
        (lambda x: x["category"] == "escalate", get_rag_chain("escalation")),
        get_rag_chain("escalation")  # <--- Add default branch
    )
)
